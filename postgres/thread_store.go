package postgres

import (
	"fmt"

	"github.com/google/uuid"
	"github.com/jaynewdee/gogoswerver/entity"
	"github.com/jmoiron/sqlx"
)

type ThreadStore struct {
	*sqlx.DB
}

func (s *ThreadStore) Thread(id uuid.UUID) (entity.Thread, error) {
	var t entity.Thread

	if err := s.Get(&t, `SELECT * FROM threads WHERE id = $1`, id); err != nil {
		return entity.Thread{}, fmt.Errorf("error getting threads: %w", err)

	}

	return t, nil
}

func (s *ThreadStore) Threads() ([]entity.Thread, error) {
	var ts []entity.Thread

	if err := s.Select(&ts, `SELECT * FROM threads`); err != nil {
		return []entity.Thread{}, fmt.Errorf("error getting threads: %w", err)
	}

	return ts, nil
}

func (s *ThreadStore) CreateThread(t *entity.Thread) error {
	if err := s.Get(t, `INSERT INTO threads VALUES ($1, $2, $3) RETURNING *`,
		t.ID,
		t.Title,
		t.Description,
	); err != nil {
		return fmt.Errorf("error creating thread: %w", err)
	}

	return nil
}

func (s *ThreadStore) UpdateThread(t *entity.Thread) error {
	if err := s.Get(t, `UPDATE threads SET title = $1, description = $2 WHERE id = $3`,
		t.Title,
		t.Description,
		t.ID,
	); err != nil {
		return fmt.Errorf("error updating thread: %w", err)
	}

	return nil
}

func (s *ThreadStore) DeleteThread(id uuid.UUID) error {
	if _, err := s.Exec(`DELETE FROM threads WHERE id = $1`, id); err != nil {
		return fmt.Errorf("error deleting thread: %w", err)
	}

	return nil
}
